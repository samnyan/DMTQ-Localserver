...
import moe.msm.dmtqserver.exception.MethodNotSupportException;
...
public class DMQHandler implements BaseHandler {
    ...
    private static final Logger logger = LoggerFactory.getLogger(DMQHandler.class);

    private final ServerConfig config;
    private final GameDatabaseService dbService;
    private final StaticFileService fileService;

    public DMQHandler(ServerConfig config, GameDatabaseService dbService, StaticFileService fileService) {
        this.config = config;
    ...
    private JSONObject gameGetSongUrl(JSONArray params) throws JSONException  {
        ...
        String ASSET_HOST = config.getAssetAddress();
    ...
    private JSONArray getMusicList() {
        try {
            File file = fileService.getFileByPath("songList.json");
            String json = StreamUtil.readAllLine(new FileInputStream(file));
            return new JSONArray(json);
        } catch (IOException | JSONException e) {
            logger.warn("songList.json not found, use default value");
            return null;
        }
    }
}
